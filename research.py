    ...
    try:
        ...
        with col1: 
            value_x = st.selectbox(
                "Value Of X", set(df.columns)
            )
        with col2:
            value_y = st.selectbox(
                "Value Of Y", set(df.columns), index=1
            )
        with col3:
        ...
        with col1: 
            value_x = st.selectbox(
                "Independent 1", set(df.columns)
            )
        with col2:
            value_y = st.selectbox(
                "Independent 2", set(df.columns), index=1
            )
...
def perform_t_tests(df: pl.DataFrame, value_x, value_y):
    ...
    t_stat, p_value = ttest_ind(group_1[value_y].to_numpy(), group_2[value_y].to_numpy())
...
def get_UN_data():
    ...
    data = {
        ...
        'Resting Heart Rate': [60, 62, 61, 63, 64],
        'Stress': [35, 36, 34, 33, 32],
        'Sleep': [7.1, 7.2, 7.3, 7.4, 7.5],
        'Daily_score': [23, 24, 25, 26, 27],
        'Actual_Steps': [10000, 12000, 11000, 13000, 12500]
    }
